...
) -> Result<Vec<(BigDecimal, String)>, MyError> {
    debugln!("form: '{}'", form);
    if form.contains('\n') {
        return Err(MyError::MultipleLine);
    }
    if form.trim() == ":q" {
        ...
        match ast_or_cmd {
            NodeOrCmd::Node(ast_root) => match calc(*ast_root, vars) {
                Ok(result) => {
                    ...
                    match out_from {
                        OutpuFormat::Default => {
                            res.push((result.clone(), num_formatter(&result, num_of_digit)));
                        }
                        OutpuFormat::Hex => {
                            res.push((result.clone(), num_hex_formatter(&result, num_of_digit)));
                        }
                        OutpuFormat::Dec => {
                            res.push((result.clone(), num_formatter(&result, 0)));
                        }
                        OutpuFormat::Bin => {
                            res.push((result.clone(), num_bin_formatter(&result, num_of_digit)));
                        }
                        OutpuFormat::Oct => {
                            res.push((result.clone(), num_oct_formatter(&result, num_of_digit)));
                        }
                    }
                }
                Err(e) => return Err(e),
